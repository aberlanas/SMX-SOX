...
** La práctica hace al maestr@.
   ...
   Si un =script= nada más empezar crea una carpeta en la ruta =C:\TMP\Canada=

   #+BEGIN_SRC shell
   New-Item -Type Directory "C:\TMP\Canada"
   # Doing stuff here ^_^
   # ...
   #+END_SRC
   
   Si lo ejecutamos varias veces, /cada vez/ intentará crear la carpeta =C:\TMP\Canada=
   ...
   $RUTA_A_CANADA="C:\TMP\Canda"

   # Sanity Checks
   if (Test-Path $RUTA_A_CANADA) {
     Write-Host " * El directorio ya existe ... "
     Remove-Item -Force -Recurse $RUTA_A_CANADA
   fi

   New-Item -Type Directory $RUTA_A_CANADA
